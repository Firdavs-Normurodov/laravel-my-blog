...
@section('content')
    ...
    <section class="relative pt-36 pb-20 mb-20 bg-my-light ">
        <img src="https://images.unsplash.com/photo-1732219516085-1de5f41d0e94?q=80&w=2036&auto=format&fit=crop&ixlib=rb-4.0.3&ixid=M3wxMjA3fDB8MHxwaG90by1wYWdlfHx8fGVufDB8fHx8fA%3D%3D"
            alt="cover-image" class="w-full absolute top-0 left-0 z-0 h-60 object-cover">
        <div class="w-full max-w-7xl mx-auto px-6 md:px-8">
                ...
                <div class="flex items-center gap-4">
                    ...
                    @auth
                        ...
                        <form method="POST" action="{{ route('logout') }}"
                            class="rounded-full border border-solid border-red-600 bg-red-600 py-3 px-4 text-sm font-semibold text-white whitespace-nowrap shadow-sm shadow-transparent transition-all duration-500 hover:shadow-gray-200 hover:bg-indigo-700 hover:border-indigo-700">

                            @csrf
                            <button type="submit" class="text-my-light">Logout</button>
                        </form>
                    @endauth
            ...
            <h3 class="text-center font-manrope font-bold text-3xl leading-10 text-gray-900 mb-3">{{ $user->name }}
                {{ $user->surname }}</h3>
